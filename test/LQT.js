...
const deployWithoutLGE = deployments.createFixture(async ({ deployments, ethers }, options) => {
  ...
  const pad = await ethers.getContract("LQT");
...
describe("BSCPad Test Suite", () => {
  ...
  describe("Deployment", () => {
    ...
    it("name should be LQT Token", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.name()).to.be.equal("LQT Token");
    });

    it("symbol should be LQT", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.symbol()).to.be.equal("LQT");
    });
    ...
    it("total supply should be 50 million tokens", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.totalSupply()).to.be.equal(tokens("50000000"));
    });

    it("deployer should have the total initial supply", async () => {
      const { pad, deployer } = await deployWithLGE();
      expect(await pad.balanceOf(deployer.address)).to.be.equal(tokens("50000000"));
    });

...
contract Marketplace is Ownable {
    ...
    function putOnSale(
        ...
        require(
            Collection(_collection).balanceOf(msg.sender, _id) >= _amount,
            "Error"
        );
    ...
    function transferWithFees(uint256 _position, uint256 _amount) internal {
        ...
        uint256 fees =
        transferFees(position.nft, price).add(
            getFee(price, marketplaceBeneficiaryFee)
        );
        payable(position.owner).transfer(price.sub(fees));
    }

    function transferFees(nftToken memory _nft, uint256 _price)
    internal
    returns (uint256)
    {
        Collection collection = Collection(_nft.collection);
        uint256 result = 0;
        uint256[] memory fees = collection.getFeeBps(_nft.id);
        address payable[] memory recipients =
        collection.getFeeRecipients(_nft.id);
